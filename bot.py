...
db = sqlite3.connect('datas', check_same_thread=False, detect_types=sqlite3.PARSE_DECLTYPES)
try:
    cursor = db.cursor()
    cursor.execute('''
...
def beatifulize(shift):
    ...
    msg =  str(week[begin.weekday()]) + ' ' + str(begin.day) + '/' + str(begin.month) + ' : \n' + \
                 str(begin.hour) + ':' + str(begin.minute) + '-' + \
                 str(end.hour) + ':' + str(end.minute)
    if shift[3] != None:
        beginSec = shift[3]
        endSec = shift[4]
        msg = msg + "\n" + str(beginSec.hour) + ':' + str(beginSec.minute) + '-' + \
...
def addShift(msg):
    ...
    #we split begin, end hours and minutes into integer that we arrange in datetime.time
    ...
    if len(parts) == 5:
        ...
        beginSec = None
        endSec = None
    #then we combine day month hours and minutes in datetime, and if end h < begin h, we add a day
    ...
    if len(parts)==5:
        ...
        hours = end - begin
        print("hours = ", hours.seconds/3600)
